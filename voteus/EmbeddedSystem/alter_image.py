...
박종수
2020.02.10
rotate_Image(imgPath, x,y,xx,yy, r,desPath):
obliterate_Image(imgPath, x,y,xx,yy, r,desPath):
...
desPath : 저장 경로
'''
def rotate_Image(imgPath, x,y,xx,yy, r,desPath):
    img = Image.open(imgPath)

    sub_image = img.crop(box=(x, y, xx, yy)).rotate(r)
...
desPath : 저장 경로
'''
def obliterate_Image(imgPath, x,y,xx,yy, r,desPath):
    img = Image.open(imgPath)
    gaussianBlur = ImageFilter.GaussianBlur(3)
    sub_image = img.crop(box=(x, y, xx, yy)).filter(gaussianBlur)
