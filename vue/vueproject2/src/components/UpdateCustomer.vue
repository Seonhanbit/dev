        ...
        <table class="content_table">
          ...
          <tr>
            <td colspan="2" style="height:50px; text-align:center;">
              <button type="submit" name="button">사원 수정하기</button>
            </td>
          </tr>
        </table>
      </form>
...
export default {
  ...
  methods: {
    updateEmployee(){
      ...
      if(this.dept_id==''){alert('부서를 선택하세요.'); return;}
      if(this.mailid==''){alert('이메일을 선택하세요.'); return;}
      if(this.manager_id==''){alert('관리자를 선택하세요.'); return;}
      if(this.salary==''){alert('월급을 입력하세요.'); return;}
      if(this.start_date==''){alert('입사일을 선택하세요.'); return;}
      if(this.title==''){alert('직책을 선택하세요.'); return;}
       http
        .post("/updateEmployee", {
